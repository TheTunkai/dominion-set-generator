...
class DominionCard extends Model
{
    ...
    {
        return DominionCard::withName($request->name != null ? $request->name : '')
            ->withCost($request->cost)
            ->withEffects($request->effects != null ? $request->effects : '')
            ->withTypes($request->types != null ? $request->types : '')
            ->orderBy('name')
            ->simplePaginate(10);
    }
    ...
    public function scopeWithCost(Builder $query, int|null $cost = null)
    {
